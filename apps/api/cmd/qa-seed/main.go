...
func main() {
	...
	// Start single transaction for all seed writes
	var cancelSeed context.CancelFunc
	ctx, cancelSeed = context.WithTimeout(context.Background(), 5*time.Minute)
	defer cancelSeed()

	tx, err := dbPool.Begin(ctx)
	if err != nil {
		log.Fatal("Failed to begin database transaction:", err)
	}
	defer func() {
	...
	// 3. Create Host Owner, Profile, Venue, Court, Sport
	var sportID string
	err = tx.QueryRow(ctx, `
	...
	if err != nil {
		err = tx.QueryRow(ctx, `SELECT id::text FROM venues WHERE owner_profile_id = $1 AND name = 'QA Arena'`, ownerProfileID).Scan(&venueID)
		if err != nil {
			log.Fatal("Venue select failed:", err)
		}
	}
	...
	if err != nil {
		err = tx.QueryRow(ctx, `SELECT id::text FROM courts WHERE venue_id = $1 AND name = 'Court QA'`, venueID).Scan(&courtID)
		if err != nil {
			log.Fatal("Court select failed:", err)
		}
	}
	...
	`, courtID)
	if err != nil {
		log.Fatal("Operating hours insert failed:", err)
	}
