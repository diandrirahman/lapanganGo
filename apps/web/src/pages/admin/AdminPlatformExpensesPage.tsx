...
import { PLATFORM_FINANCE_TIME_ZONE } from '../../lib/platformExpenseForm';
...
const JournalList: React.FC<{ journals: PlatformJournal[]; selectedJournalID: string; onJournalLink: (journalID: string) => void }> = ({ journals, selectedJournalID, onJournalLink }) => (
  <div data-focused-journal={selectedJournalID || undefined} onClick={(event) => {
    ...
    <div className="space-y-3 md:hidden">{journals.map((journal) => <article id={`journal-${journal.id}`} key={journal.id} className="min-w-0 rounded-xl border border-slate-200 bg-white p-4 shadow-sm"><div className="flex min-w-0 items-start justify-between gap-3"><div className="min-w-0"><h2 className="break-words font-semibold text-slate-900">{journal.event_type}</h2><p className="mt-1 break-all text-xs text-slate-500">{shortID(journal.id)} · {journal.entry_count} entries</p></div><span className="shrink-0 text-xs font-semibold text-emerald-700">Balanced</span></div><p className="mt-3 text-sm text-slate-700">{formatDateTime(journal.effective_at)} · {formatRupiah(journal.debit_total_rupiah)}</p><p className="mt-2 text-xs text-amber-700">{journal.reverses_journal_id ? <a href={`#journal-${journal.reverses_journal_id}`} className="hover:underline">Reversal of {shortID(journal.reverses_journal_id)}</a> : journal.reversed_by_journal_id ? <a href={`#journal-${journal.reversed_by_journal_id}`} className="hover:underline">Reversed by {shortID(journal.reversed_by_journal_id)}</a> : <span className="text-slate-400">—</span>}</p></article>)}</div>
  </div>
);
...
export const AdminPlatformExpensesPage: React.FC = () => {
  ...
  const fetchData = useCallback(async () => {
    ...
    if (tab === 'expenses') setExpensePage(null);
    else setJournalPage(null);
    try {
      if (tab === 'expenses') {
  ...
  return <div className="mx-auto max-w-7xl space-y-6">
    ...
    <div className="rounded-xl border border-indigo-200 bg-indigo-50 p-4 text-sm text-indigo-950" role="note" aria-label="Platform Finance simulation mode"><p className="font-semibold">MODE SIMULASI</p><p className="mt-1">Expense dan journal pada halaman ini adalah diagnostics non-LIVE. Fitur ini tidak melakukan settlement, payout, atau pemotongan komisi owner.</p></div>
