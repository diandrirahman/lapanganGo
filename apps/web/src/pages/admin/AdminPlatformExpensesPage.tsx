...
const ExpenseList: React.FC<{ expenses: PlatformExpense[]; onAction: (action: ExpenseAction) => void }> = ({ expenses, onAction }) => (
  <>
          ...
          <tbody className="divide-y divide-slate-100">
            {expenses.map((expense) => (
              <tr key={expense.id} className="hover:bg-slate-50">
                ...
                <td className="px-5 py-4 align-top text-sm text-slate-700">{expense.posted_journal_id ? <span className="font-medium text-emerald-700">Posted {shortID(expense.posted_journal_id)}</span> : <span className="text-slate-400">No journal yet</span>}{expense.void_journal_id && <p className="mt-1 text-xs text-amber-700">Void {shortID(expense.void_journal_id)}</p>}</td>
                <td className="px-5 py-4 align-top">{expense.status === 'DRAFT' ? <div className="flex flex-col items-start gap-2"><button type="button" onClick={() => onAction({ type: 'approve', expense })} className="inline-flex items-center rounded-lg bg-sky-600 px-3 py-1.5 text-xs font-semibold text-white hover:bg-sky-700"><CheckCircle2 className="mr-1.5 h-3.5 w-3.5" />Approve</button><button type="button" onClick={() => onAction({ type: 'cancel', expense })} className="inline-flex items-center rounded-lg border border-rose-200 px-3 py-1.5 text-xs font-semibold text-rose-700 hover:bg-rose-50"><XCircle className="mr-1.5 h-3.5 w-3.5" />Cancel</button></div> : <span className="text-xs text-slate-400">No actions</span>}</td>
              </tr>
            ))}
          </tbody>
        </table>
...
export const AdminPlatformExpensesPage: React.FC = () => {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      if (tab === 'expenses') {
        ...
        const response = await adminApi.getPlatformJournals({ page, limit: PAGE_LIMIT, ...(startDate ? { start_date: startDate } : {}), ...(endDate ? { end_date: endDate } : {}), ...(eventType.trim() ? { event_type: eventType.trim().toUpperCase() } : {}) }, { signal: controller.signal });
        if (requestID !== requestIDRef.current) return;
  ...
  }, [category, endDate, eventType, page, startDate, status, tab]);

  useEffect(() => { void fetchData(); return () => controllerRef.current?.abort(); }, [fetchData]);
  const totalPages = tab === 'expenses' ? expensePage?.total_pages ?? 0 : journalPage?.total_pages ?? 0;

  const resetFilters = () => { setStatus(''); setCategory(''); setStartDate(''); setEndDate(''); setEventType(''); setPage(1); };
  const switchTab = (nextTab: 'expenses' | 'journals') => { setTab(nextTab); setPage(1); setError(null); };
  ...
  return <div className="mx-auto max-w-7xl space-y-6">
    ...
    <CreateExpenseModal isOpen={isCreateOpen} onClose={() => setIsCreateOpen(false)} onCreated={() => { setIsCreateOpen(false); setTab('expenses'); setPage(1); void fetchData(); }} />
    <ExpenseActionModal isOpen={isActionOpen} action={actionDraft} onClose={() => setIsActionOpen(false)} onCompleted={() => { setIsActionOpen(false); setActionDraft(null); setTab('expenses'); setPage(1); void fetchData(); }} onConflict={() => { setIsActionOpen(false); setActionDraft(null); void fetchData(); }} />
  </div>;
};
