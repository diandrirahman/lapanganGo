...
export async function fetchVenues(page: number = 1, limit: number = 10, filters?: { city?: string; sport_id?: string; facility_ids?: string[]; min_price?: number; max_price?: number }): Promise<PaginatedResponse<Venue>> {
    ...
    return new Promise((resolve) => {
      setTimeout(() => {
        resolve({
          ...
          end_time: '16:00',
          total_price: 300000,
          status: isApproved ? 'CONFIRMED' : 'PENDING_PAYMENT',
          created_at: new Date().toISOString(),
          updated_at: new Date().toISOString(),
        } as unknown as Booking);
      }, 500);
