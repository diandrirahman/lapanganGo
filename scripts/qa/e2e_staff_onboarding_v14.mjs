...
const outDir = 'D:/project/lapangGo/outputs/qa/staff-onboarding-v1.4';
mkdirSync(outDir, { recursive: true });

const runId = `${Date.now()}`;
const password = 'QaStaffV14!2026';
const report = {
  run_id: runId,
...
async function main() {
  ...
  await request('PUT', `/owner/staff/${staff.id}/venues`, {
    ...
    const failedStaffLogin = await request('POST', '/auth/login', {
    expected: [403, 401],
    body: { email: staffEmail, password },
    name: 'staff login before setup password',
  });
  record('Staff login ditolak sebelum setup password', failedStaffLogin.status >= 400 ? 'PASS' : 'FAIL');

  const inviteUrl = new URL(staff.invite_url);
  const inviteToken = inviteUrl.searchParams.get('token');
  
  await request('POST', '/staff/setup-password', {
    expected: [200],
    body: { token: inviteToken, password },
    name: 'staff setup password',
  });
  record('Staff berhasil setup password dengan invite token', 'PASS');
